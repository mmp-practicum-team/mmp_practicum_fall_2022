...
- В частности, код должен проходить автоматическую проверку стиля [ссылка](https://github.com/PyCQA/pycodestyle/blob/main/pycodestyle.py). Скрипт запускается из командной строки так: python3 pycodestyle.py <ваш скрипт>. Код, вызывающий предупреждения, может дополнительно штрафоваться.
